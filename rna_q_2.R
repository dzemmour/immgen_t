...
tmp$order <- ifelse(tmp$outliers_nGenes=="FALSE", 1, 2)
p = ggplot(tmp) + geom_point(aes(dim1, dim2, color = outliers_nGenes), size = 1,order=order) + 
  theme_bw() + ggtitle(label = sprintf("umap_preRNAqc, %s", "outliers_nGenes")) + scale_color_manual(values = c("black", "red"))
print(p)
print(p + facet_wrap(~HTO_classification, ncol = 2))

tmp$order <- ifelse(tmp$outliers_deadcells=="FALSE", 1, 2)
p = ggplot(tmp) + geom_point(aes(dim1, dim2, color = outliers_deadcells), size = 1,order=order ) + 
   theme_bw() + ggtitle(label = sprintf("umap_preRNAqc, %s", "outliers_deadcells")) + scale_color_manual(values = c("black", "red"))
print(p)
print(p + facet_wrap(~HTO_classification, ncol = 2))
