...
htos <- as.data.frame(so@assays$HTO@counts)
hto_table<-data.frame(matrix(ncol = 1, nrow =nrow(htos) ))
for (n in 1:length(rownames(hto_table))) {
...
for(i in which(so$HTO_classification.simplified=="Doublet")){
  ...
  so_max <- so@assays$HTO@counts[, rownames(so@meta.data)[so@meta.data$HTO_classification == names(which.max(so@assays$HTO@counts[,i]))]]
  median_max <- median(so_max[rownames(so_max) == names(which.max(so@assays$HTO@counts[,i]))])
  ...
  so_max2 <-  so@assays$HTO@counts[, rownames(so@meta.data)[so@meta.data$HTO_classification == names(which.max(so@assays$HTO@counts[,i][-which.max(so@assays$HTO@counts[,i])]))]]
  median_max2 <- median(so_max2[rownames(so_max2) == names(which.max(so@assays$HTO@counts[,i][-which.max(so@assays$HTO@counts[,i])]))])
  
  if(!is.na(median_max2)) { 
    norm_max2 <- max2_hash/median_max2
    }
  else norm_max2 = 0
...
so = so[,so$HTO_classification.global %in% c("Singlet", "Rescued", "Rescued_Doublet")]
