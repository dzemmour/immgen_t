...
filtered_data_seurat_adt<-CreateSeuratObject(counts = filtered_data_seurat,assay = "ADT")
...
if (Method == 'Manual'){
  ...
  data<- as.data.frame(umicount_antibodies)
  data[data==0] <- NA
  data <-na.omit(data)
...
#umicount_gene <- (as.matrix(colSums(data1$`Gene Expression`)))
umicount_antibodies <- (as.matrix(colSums(data1)))
#data<- as.data.frame(cbind(umicount_gene,umicount_antibodies))
data<- as.data.frame(umicount_antibodies)
data[data==0] <- NA
data <-na.omit(data)
...
print('HI')
add_data <- cbind(add_data, 'Cells')
colnames(add_data)[2] <- "Classification"
print('HI2')
subtract_data <- cbind(subtract_data, 'Empty Droplets')
colnames(subtract_data)[2] <- "Classification"
print('HI3')
data_total <- rbind(add_data, subtract_data)
print('HI4')
