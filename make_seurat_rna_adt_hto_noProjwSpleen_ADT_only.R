...
so = CreateSeuratObject(counts = htos_adt,strip.suffix = T)
...
tmp = tmp %>% group_by(HTO_classification.simplified) %>% summarize(ncells = n(),  mean_nCount_HTO = round(mean(nCount_HTO))) %>% as.data.frame()
