...
path_to_rna = args[2] #path to RNA
path_to_hto_adt = args[3] #path_to_hto_adt 
path_to_hto_adt_remove_from_analysis = args[4] #path_to_hto_adt_remove_from_analysis = "hash_adt_rows_to_remove_from_analysis.txt"
spleen<- args[5] #hashtag for spleen control to remove from t cells
...
umis = umis[,joint.bcs]
htos = htos_adt[rownames(htos_adt)[grepl("Hash|HT[0-15]", rownames(htos_adt))],joint.bcs]
adt = htos_adt[rownames(htos_adt)[!grepl("Hash|HT[0-15]|unmapped", rownames(htos_adt))],joint.bcs]
message("These are the hashes: ")
print(htos)

hto = read.table(path_to_hto_adt_remove_from_analysis, header = F)[,1]
htos <- htos[!rownames(htos) %in% hto,]
...
so<- subset(x= so, idents= rownames(spleen), invert = TRUE)
}
