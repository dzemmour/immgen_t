...
mkdir -p GEX
mkdir -p FBC
Rscript $1/filter2.R combined_sample_withtransgenes/outs/raw_feature_bc_matrix combined_sample_withtransgenes/outs/filtered_feature_bc_matrix $2 $IGT Automatic $RNA_Cutoff $ADT_Cutoff > filter.log 2> filter.err
 
Rscript $1/make_seurat_rna_adt_hto_noProj.R $project_description GEX/filtered_feature_bc_matrix FBC/filtered_feature_bc_matrix $2 $spleen  > make_seurat_rna_adt_hto.log 2> make_seurat_rna_adt_hto.err 
