...
fasta_name <- paste(experiment_name, "allCells_productiveANDnonproductiveTCR_pairedANDunpaired_trv_cdr3", sep = "_")

sc <- readRDS(seurat_obj_name)

tcr <- read.csv("TCR/outs/all_contig_annotations.csv", sep=",")
...
filt_meta <- meta[!is.na(meta$full_a_seq_contig1) | !is.na(meta$full_b_seq_contig1),]
...
saveRDS(clono_seurat, "seuratobject_withHTOADT_singlet_fullTCRinfo_productiveANDnonproductive_pairedANDunpaired.Rds")
