...
message("Plot QC")
png("seuratobject_withHTOADT_singlet_preRNAfiltering-1.png",width=1600, height=1600)
...
png("seuratobject_withHTOADT_singlet_preRNAfiltering-2.png",width=1600, height=1600)
...
png("seuratobject_withHTOADT_singlet_preRNAfiltering-3.png",width=1600, height=1600)
tmp = data.frame(so@meta.data)
...
png("seuratobject_withHTOADT_singlet_preRNAfiltering-4.png",width=1600, height=1600)
tmp = data.frame(so@meta.data, dim1 = so@reductions$umap_preRNAqc@cell.embeddings[,1], dim2 = so@reductions$umap_preRNAqc@cell.embeddings[,2])
p = ggplot(tmp) + geom_point(aes(dim1, dim2, color = outliers_nGenes), size = I(0.5)) + 
  theme_bw() + ggtitle(label = sprintf("umap_preRNAqc, %s", "outliers_nGenes")) + scale_color_manual(values = c("grey", "red"))
#print(p)
