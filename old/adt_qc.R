...
Backgate = function(so = so, x_adt = "CD3-GTATGTCCGCTCGAT", y_adt = "B220-CCTACACCTCATAAT", x_adt_bckgplots = "CD3-GTATGTCCGCTCGAT", size_dot_bckgplots = 1, plot_marginals = T, gate = NULL) {
require(gatepoints)
require(ggplot2)
require(ggrastr)
...
meta.features = read.table(file = path_to_metadata_adt, header = T, row.names=NULL, sep = ",")
...
pdf("adt_qc_plot3_UMAP_clustering_prefiltering.pdf", 15, 15, useDingbats=FALSE) 
...
message(sprintf("Flag autofluorescent cells with cells that have strong signal for >= %s isotype contols ...", n_isotype_ctrl_signal_to_flag))
...
tmp = tmp %>% group_by(HTO_classification) %>% summarize(ncells = n(),  mean_nCount_RNA = round(mean(nCount_RNA)), mean_nFeature_RNA = round(mean(nFeature_RNA)), mean_nCount_HTO = round(mean(nCount_HTO)), mean_nCount_ADT = round(mean(nCount_ADT)),  ncells_outliers_lowCountADT = length(which(so$lowCountADT)), ncells_outliers_autofluorescence = length(which(autofluo))) %>% as.data.frame()
